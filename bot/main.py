...
async def audit(
        ...
        if member.get("alliance_position", "") != "APPLICANT":
            if type == "activity":
                ...
                if member.get("spies", 0) < 50 and member.get("spies_today", 0) < max_spies:
                    ...
                    audit_results.append(result)
                    needers.append(f"@{member.get('discord','N/A')}")
            elif type == "project" or type == "projects":
            ...
            elif type == "bloc" or type == "block" or type == "color":
                member = get_data.GET_NATION_DATA(member.get("id", 0), API_KEY)
                summary = "### The Following People Need To Switch To Purple"

                if member.get("color", "") != "purple":
                    nation_url = f"https://politicsandwar.com/nation/id={member['id']}"
                    result = (
                        f"**Leader:** [{member['leader_name']} - {member['nation_name']}]({nation_url})\n"
                        f"**Discord:** {member.get('discord', 'N/A')}\n"
                        f"**Color:** {member.get('color', 'N/A')}\n"
                    )
                    audit_results.append(result)
                    needers.append(f"@{member.get('discord','N/A')}")
            elif type == "military" or type == "mil":
                member = get_data.GET_NATION_DATA(member.get("id", 0), API_KEY)
                summary = "### The Following People Need To Buy Military"
                ...
                for city in member.get("cities", []):
                    units["soldiers"] += (city.get("barracks", 0) * 3000) 
                    units["tanks"] += (city.get("factory", 0) * 250)
                    units["aircraft"] += (city.get("hangar", 0) * 15)
                    units["ships"] += (city.get("drydock", 0) * 5)

                units["soldiers"] += (member.get("military_research", {}).get("ground_capacity", 0) * 3000)
                units["tanks"] += (member.get("military_research", {}).get("ground_capacity", 0) * 250)
                units["aircraft"] += (member.get("military_research", {}).get("air_capacity", 0) * 15)
                units["ships"] += (member.get("military_research", {}).get("naval_capacity", 0) * 5)

                if member.get("soldiers", 0) < (0.8 * units["soldiers"]) or member.get("tanks", 0) < (0.8 * units["tanks"]) or member.get("aircraft", 0) < (0.8 * units["aircraft"]) or member.get("ships", 0) < (0.8 * units["ships"]):
                    nation_url = f"https://politicsandwar.com/nation/id={member['id']}"
                    result = (
                        f"**Leader:** [{member['leader_name']} - {member['nation_name']}]({nation_url})\n"
                        f"**Discord:** {member.get('discord', 'N/A')}\n"
                        f"**Military:**\n"
                        f"**Soldiers:** {member.get('soldiers', 0)}\n"
                        f"**Tanks:** {member.get('tanks', 0)}\n"
                        f"**Aircraft:** {member.get('aircraft', 0)}\n"
                        f"**Ships:** {member.get('ships', 0)}\n"
                    )
                    audit_results.append(result)
                    needers.append(f"@{member.get('discord','N/A')}")
                ...
                for cities in member.get("cities", []):
                    mmr = {
                        "barracks": cities.get("barracks", 0),
                        "factory": cities.get("factory", 0),
                        "hangar": cities.get("hangar", 0),
                        "drydock": cities.get("drydock", 0)
                    }

                    member_mmr.append({cities.get("name", "N/A"): mmr})

                # check to see if any of the nation's cities are violating their required MMR
                if len(member.get("cities")) < 15:
                    for city in member_mmr:
                        for city_name, city_mmr in city.items():
                            if city_mmr.get("barracks", 0) < required_mmr["Raider"]["barracks"] or city_mmr.get("factory", 0) < required_mmr["Raider"]["factory"] or city_mmr.get("hangar", 0) < required_mmr["Raider"]["hangar"] or city_mmr.get("drydock", 0) < required_mmr["Raider"]["drydock"]:
                                nation_url = f"https://politicsandwar.com/nation/id={member['id']}"
                                result = (
                                    f"**Leader:** [{member['leader_name']} - {member['nation_name']}]({nation_url})\n"
                                    f"**Discord:** {member.get('discord', 'N/A')}\n"
                                    f"**City:** {city_name}\n"
                                    f"**Barracks:** {city_mmr.get('barracks', 0)}\n"
                                    f"**Factory:** {city_mmr.get('factory', 0)}\n"
                                    f"**Hangar:** {city_mmr.get('hangar', 0)}\n"
                                    f"**Drydock:** {city_mmr.get('drydock', 0)}\n"
                                )
                                audit_results.append(result)
                                needers.append(f"@{member.get('discord','N/A')}")
                else:
                    for city in member_mmr:
                        for city_name, city_mmr in city.items():
                            if city_mmr.get("barracks", 0) < required_mmr["Whale"]["barracks"] or city_mmr.get("factory", 0) < required_mmr["Whale"]["factory"] or city_mmr.get("hangar", 0) < required_mmr["Whale"]["hangar"] or city_mmr.get("drydock", 0) < required_mmr["Whale"]["drydock"]:
                                nation_url = f"https://politicsandwar.com/nation/id={member['id']}"
                                result = (
                                    f"**Leader:** [{member['leader_name']} - {member['nation_name']}]({nation_url})\n"
                                    f"**Discord:** {member.get('discord', 'N/A')}\n"
    ...
    info(f"Audit completed for {len(members)} members of alliance: {ALLIANCE_ID}", tag="AUDIT")
...
@bot.tree.command(name="purge", description="Find nations on Purple to purge.")
async def purge(interaction: discord.Interaction):
    await interaction.response.defer()

    info(f"Purge command executed by {interaction.user} in {interaction.channel}", tag="COMMAND EXECUTION")

    try:
        nations = get_data.GET_PURGE_NATIONS(API_KEY)
        nations = [nation for nation in nations if nation.get("alliance", {}).get("id", "") != ALLIANCE_ID]
        nations = [nation for nation in nations if len(nation.get("cities", 0)) < 15]

        print(f"nations: {nations}")
    except Exception as e:
        error(f"Error fetching purge nations: {e}", tag="PURGE")
        await interaction.followup.send("Error fetching purge nations. Please try again later.")
        return

    if not nations:
        await interaction.followup.send("No nations found on Purple.")
        return
    
    # Create a list of nations with their IDs and names.
    nation_list = []
    for nation in nations:
        if len(nation['cities']) < 15:
            ...
            result = (
                f"**Nation:** [{nation['nation_name']} - {nation['leader_name']}]({nation_url})\n"
                f"**Alliance:** {nation.get('alliance', {}).get('name', 'None')}\n"
                f"**Score:** {nation.get('score', 'N/A')}\n"
                f"**Soldiers:** {nation.get('soldiers', 0)}\n"
                f"**Tanks:** {nation.get('tanks', 0)}\n"
                f"**Aircraft:** {nation.get('aircraft', 0)}\n"
                f"**Ships:** {nation.get('ships', 0)}\n"
                f"**Defensive Wars:** {nation.get('defensive_wars_count', 0)}\n"
            )
            nation_list.append(result)
    # Create a paginator for the nation list.
    paginator = ActivityPaginator(nation_list)
    await interaction.followup.send(embed=paginator.get_embed(), view=paginator)
